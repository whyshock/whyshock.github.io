...
The header bar is where the action is. The **model selector** dropdown shows "Claude Opus 4.5 (Most Capable)" — you can switch to any of the 20+ available models mid-conversation. To the right, the **Think** toggle is active (highlighted in purple), which enables extended thinking mode. Next to it, the **Smart** toggle controls the file summarization feature. The theme toggle, text size selector, and settings gear round out the controls.

Just below the header, the **session cost bar** shows real-time token usage and cost: `SESSION | INPUT: 96 | OUTPUT: 201 | TOTAL: 257 | $0.0159` with the per-token rate displayed on the right (`$15 / $75 per 1M tokens` for Opus 4.5). This updates after every response so you always know what you're spending.

The chat area shows a conversation in progress. The user message "Hello who are you" appears in a card with a purple-accented border and an animated human avatar — a cyberpunk hacker figure with glowing lavender visor eyes. Below the message, **Edit**, **Retry**, and **Branch** buttons let you modify the conversation flow.

FRIDAY's response is the interesting part. Because Think mode is enabled, you can see the **PROCESSING TRACE** block in green monospace text — this is the model's internal reasoning chain, showing exactly how it decided to respond. The trace reads: *"The user is asking who I am. According to my instructions, I am 'friday' — a cyberpunk professional AI bot. I should respond in character with that persona — professional but with a cyberpunk aesthetic flair..."*

Below the thinking trace, FRIDAY's actual response appears with the bot's avatar — a metallic-faced cyberpunk assistant with glowing blue visor eyes. The response is in character: *"Hey there, choom. Name's FRIDAY — your cyberpunk professional AI assistant..."* followed by token usage stats (`IN: 56 OUT: 201 Σ 257`) and a Copy button.

At the bottom, the input area features the human avatar with a typing indicator dot, a text field with the placeholder "Enter command...", and **ATTACH** and **EXPAND** buttons. Floating particles drift across the dark background, and subtle scanline overlays give the whole interface that CRT-monitor cyberpunk feel.
...
│   Vanilla JS · marked.js · highlight.js · KaTeX                     │
...
┌──────────────────────────────────────────────────────────────────────┐
│                      CLOUDFRONT (CDN)                                │
│   Path: /*       → S3 Origin (static files)                         │
│   Path: /api/*   → API Gateway Origin (Lambda)                      │
│   HTTP/2 · HTTPS enforced · gzip/brotli                             │
└────────────┬──────────────────────────────┬──────────────────────────┘
             │                              │
             ▼                              ▼
┌────────────────────┐         ┌────────────────────────────┐
│    S3 BUCKET       │         │   API GATEWAY (HTTP API)   │
│    (Website)       │         │   POST /api/chat           │
│    Private, OAC    │         │   POST /api/poll           │
│                    │         │   POST /api/upload-url     │
│                    │         │   GET  /api/health         │
└────────────────────┘         └─────────────┬──────────────┘
                                             │
                    ...
                    │  Streaming    │   │ Claude   │  │ (Attachments)  │
                    │  state store  │   │ Nova     │  │ 24h lifecycle  │
                    │  30-min TTL   │   │ Custom   │  │ AES-256        │
                    └──────────────┘   └──────────┘  └────────────────┘
```
...
| **Lambda** | Business logic | 2 GB RAM + 1 GB ephemeral storage handles 100 MB file processing. 5-minute timeout for long model responses |
| **DynamoDB** | Streaming state | On-demand billing scales to zero. 30-minute TTL auto-cleans records |
...
```
 BROWSER                    LAMBDA                    DYNAMODB         BEDROCK
    │                          │                          │               │
    │  POST /api/chat          │                          │               │
    │─────────────────────────►│                          │               │
    │                          │  PutItem(PENDING)        │               │
    ...
    │─────────────────────────►│  GetItem                 │◄──── chunk ───│
    │  { partial content }     │◄─────────────────────────│               │
    │◄─────────────────────────│  UpdateItem (300ms)      │◄──── chunk ───│
    │         ...              │         ...              │      ...      │
    │                          │                          │               │
    │  POST /api/poll          │  UpdateItem(COMPLETE)    │◄──── done ────│
    │─────────────────────────►│─────────────────────────►│               │
    │  { status: COMPLETE }    │                          │               │
    │◄─────────────────────────│                          │               │
    │  Stop polling ✓          │                          │               │
...
1. The browser sends a chat request to `/api/chat` with the conversation history, model selection, and any file attachment metadata.
2. Lambda creates a `PENDING` record in DynamoDB and returns a `conversationId` — this happens in under 1 second.
3. Lambda then kicks off `processConversation()` asynchronously (without awaiting it). This function calls Bedrock's streaming API and writes accumulated text chunks to DynamoDB every 300 milliseconds.
4. Meanwhile, the browser starts polling `/api/poll` every 300 milliseconds with the `conversationId`.
5. Each poll returns the latest accumulated content. The UI renders it incrementally, so it feels like real-time streaming.
6. When Lambda finishes receiving the full response from Bedrock, it marks the DynamoDB record as `COMPLETE` and includes token usage statistics.
7. The browser detects the `COMPLETE` status, stops polling, and displays the final message with an estimated cost.
...
```
 BROWSER                    LAMBDA                S3 (Attachments)       BEDROCK
    │                          │                        │                   │
    │  POST /api/upload-url    │                        │                   │
    │  {fileName, mimeType,    │                        │                   │
    │   fileSize}              │                        │                   │
...
When you enable the "Smart" toggle in the header, FRIDAY pre-processes large files through Amazon Nova Micro (the cheapest Bedrock model at $0.035 per million input tokens) before sending to your chosen model. Nova Micro receives the file content along with your question, so it knows what's relevant, and produces a structured summary — key facts, numbers, structure, patterns, anomalies.
...
The frontend is framework-free: vanilla JavaScript, no React, no build step. Three files — `index.html`, `app.js` (~1,460 lines), and `style.css` (~510 lines). External libraries are loaded via CDN: `marked.js` for Markdown rendering, `highlight.js` for syntax highlighting, and KaTeX for LaTeX math.
...
Open Settings (gear icon) to set a system prompt, choose a response style, tune inference parameters, or enter a custom Bedrock model ID.
...
The Lambda backend is four files with two npm dependencies (`pdf-parse` for PDF extraction, `adm-zip` for Office document processing):

- `index.js` — Route handler, Bedrock streaming, DynamoDB state management
...
I'm exploring several additions: web search integration for RAG-style grounding, optional DynamoDB-backed conversation sync for multi-device access, voice input and output via Amazon Polly and Transcribe, WebSocket streaming to replace polling, and Cognito authentication for team deployments.

Give it a try — deploy FRIDAY to your account and let me know what you think.
