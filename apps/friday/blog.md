...
│   Web Speech API (voice input) · DuckDuckGo/Brave (web search)     │
└──────────────────────────┬───────────────────────────────────────────┘
                           │ HTTPS
                           ▼
┌──────────────────────────────────────────────────────────────────────┐
...
```
 BROWSER                    LAMBDA                  DDG/BRAVE          BEDROCK
    │                          │                        │                │
    │  POST /api/chat          │                        │                │
    │  {webSearch: true}       │                        │                │
    │─────────────────────────►│                        │                │
...
When you enable the "Smart" toggle below the textarea, FRIDAY pre-processes large files through Amazon Nova Micro (the cheapest Bedrock model at $0.035 per million input tokens) before sending to your chosen model. Nova Micro receives the file content along with your question, so it knows what's relevant, and produces a structured summary — key facts, numbers, structure, patterns, anomalies.
